...
```{r}
fpout$scores <- fpout$scores %>% 
  mutate(posterior_mean = (P0 * 0) + (P1 * 1) + (P2 * 2) + (P3 * 3) + (P4 * 4) + (P5 * 5) + (P6 * 6))
fpout$scores %>% select(posterior_mean, everything()) %>% head(5)
```
...
fpout$scores %>%
  keep(is.double) %>% 
  map(mean) -> fpout$col_means

fpout$col_means <- fpout$col_means %>% unlist
...
#calcn(c(11,1,130), 1000L) #this is taking too long time everytime I knit, so I will save it to last. I hope I don't forget it.
calcn(c(11,1,130), 1L)
calcn(c(7, 3, 20), 8L)
```
...
set.seed(1)
desired_length <- 1000
tlist <- vector(mode = "list", length = desired_length)

for (i in seq_along(tlist)){
  tlist[[i]] <- rnorm(n = 10, mean = 0, sd = 2) %>% t.test()
}
...
map_dfc(tlist, "estimate") %>% 
  pivot_longer(c('V1':'V1000'), names_to="test", values_to = "mean") %>% 
  ggplot(aes(x=mean)) + geom_histogram(aes(y=..density..), bins = 50, fill='deepskyblue4')
```
...
```{r}
ggplot(data.frame(pvec_m), aes(sample=pvec_m))+
  stat_qq()+
  stat_qq_line(linetype="dashed", color="red")+
  labs(title="QQ-plot for p-values")
```

We have an S-shaped data. It indicates that we indeed have a uniform distribution. If we had a normal distribution, the data would align on smoother line.
